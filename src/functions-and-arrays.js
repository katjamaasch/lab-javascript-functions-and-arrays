...
should create a function named sum
...
function sum(anArray) {
  ...
  for (let i = 0; i < anArray.length; i++) {
    if (typeof anArray[i] == 'string') {
      sumUpStrings = sumUpStrings + anArray[i].length;
    } else if (typeof anArray[i] == 'number') {
      sumUpNumbers = sumUpNumbers + anArray[i];
...
function averageWordLength(someAr) {
  if (someAr.length == 0) {
    return null;
  }
  let calculate = 0;
  let average = 0;
  for (let i = 0; i < someAr.length; i++) {
    calculate = calculate + someAr[i].length;
  }
  average = calculate / someAr.length;
  return average;
}
...
function doesWordExist(yeayArray, word) {
  if (yeayArray.length == 0) {
    return null;
  } else if (yeayArray.indexOf(word) !== -1) {
    return true;
  } else {
    return false;
  }
...
function howManyTimes(anotherArray, word) {
